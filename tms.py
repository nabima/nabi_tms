...
class tms_picking(osv.osv):
    ...
    def onchange_km(self, cr, uid, ids, depart=0,arrivee=0, context=None):
        data = {}
        warning = False
        if arrivee > depart:
            km_estimated = arrivee - depart  or 0.0
            data.update({'km_estimated': km_estimated})
        elif arrivee == 0:
            return
            ...
            warning = {
                            'title': u'Error',
                            'message': u"Kilométrage d'arrivée doit être supérieur à %s" % depart
                        }
...
#    def onchange_quantity_travel(self, cr, uid, ids, product_id, product_qty,product_uom, product_uos, price_unit,commission_fixed, commission_value_type, travel_return):
    ...
    _columns = { 
        ...
        'state': fields.selection([
            ('draft', u'Sans Affectation'),
            ('cancel', u'Annulé'),
            ('confirmed', u'Plannifié'),
            ('assigned', u'Affecté'),
            ('charge',u'Point de chargement'),('road',u'en route'),('decharge',u'Chez client'),
            ('done', u'Terminé'),
        ...
        'invoice_state': fields.selection([
            ...
            select=True, required=True, readonly=True, track_visibility='onchange', copy=False),
